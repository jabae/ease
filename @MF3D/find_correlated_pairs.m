function  [merged_ROIs, newIDs] = find_correlated_pairs(obj, corr_thr)
%% find neuron pairs that are highly correlated
% input:
%   corr_thr: 1X3 vector, threshold for three metrics {'C', 'S', 'A'}, it merge neurons based
...
if ~exist('corr_thr', 'var') || isempty(corr_thr) || numel(corr_thr)~=3
    corr_thr = [1e-6, 0.5, 0];
end
A_thr = corr_thr(1);
C_thr = corr_thr(2);
...
A_overlap = cosine_similarity(neuron.A);
...
if length(corr_thr)>3
    max_decay_diff = corr_thr(4);
    taud = zeros(K, 1);
    for m=1:K
