function initialize_multiple(obj, Aem, segment_ids, Y, options)
...
if ~clear_results
    ...
    ids_matched = cell2mat(obj.match_status.em_indices(obj.match_status.status==1));
...
em_indices = tmp_ind;           % ids of the candidate neurons
with_quality_control = false;
white_list = [];

% normalize the spatial masks of the selected neurons
A_selected = full(Aem(:, em_indices));       % spatial masks of these neurons
...
elseif ~isempty(obj.P.sn)  % normalize data 
    Y = bsxfun(@times, double(Y), 1./obj.P.sn); 
end

if clear_results  % clear existing results
    Y = obj.reshape(Y,1);
    obj.A = [];
    obj.C = [];
    obj.labels = [];
    ...
    Y = obj.reshape(Y,1) - tmp_b*tmp_f;  % roughly estimate the background
else
    if ~isempty(obj.b)      % remove background 
        Y = obj.reshape(Y, 1)- obj.b*obj.f;
    end
    if ~isempty(obj.A)      % remove neural signal 
        Y = obj.reshape(Y,1) - obj.A*obj.C ;
    end
end

% center the fluroescence trace of each voxel 
...
while (k_new < K_new+K_pre) && (k_tried<size(A_,2))
    ...
    if strcmpi(init_method, 'tf')
        ...
        [ai_new(ind_in), ci_new] = initialize_ac_tf(Yin, Yout, ai(ind_in(:)), ci0, 20);
