%{
nda.VonMises (computed) # directional tuning
-> nda.Spike
-----
...
classdef VonMises < dj.Relvar & dj.AutoPopulate
    ...
    properties
        popRel  = nda.Spike & nda.DriftResponse;
    end
        ...
        function plot(self)
            ...
            for icell=1:length(cellKey)
                ...
                for imask=1:length(maskKey)
                    subplot(length(maskKey),1,imask)
    ...
    methods(Access=protected)
        ...
        function makeTuples(self, key)
            s = fetch(nda.DriftResponse*nda.DriftTrial & key, 'direction','response');
