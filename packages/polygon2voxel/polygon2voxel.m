...
switch(lower(mode(1:2)))
    case {'au'} % auto
        ...
        %% modified by Pengcheng Zhou, 2017
        % Make all vertices-coordinates positive
        FV.vertices= bsxfun(@minus, FV.vertices, min(FV.vertices(:)));
        scaling = (VolumeSize-1)./max(FV.vertices, [],1);
        FV.vertices = bsxfun(@times, FV.vertices, scaling) + 1; 
        %% modified by Pengcheng Zhou, 2017 
