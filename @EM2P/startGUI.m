...
handles.btn_update_cnmf = uicontrol(handles.fig_main, 'style', 'pushbutton', 'units', 'pixels', ...
    ...
    'fontsize', font_size, 'fontweight', 'bold', 'callback', 'cb_run_cnmf;');

handles.btn_save_neuron = uicontrol(handles.fig_main, 'style', 'pushbutton', 'units', 'pixels', ...
    'position', pos_btn_save_neuron, 'string', 'SAVE', 'backgroundcolor', handles.color_cyan,...
    'fontsize', font_size, 'fontweight', 'bold', 'callback', ...
    'neuron.evaluate_matching_confidence(Y, Aem, segment_ids); ease.save_MF3D();');
...
handles.text_cell_id = uicontrol(handles.fig_main, 'style', 'edit', 'units', 'pixels', ...
    ...
    'callback', 'cell_id= str2double(get(gco, ''string''));cb_show_2p_neuron;');
handles.btn_ca_next = uicontrol(handles.fig_main, 'style', 'pushbutton', 'units', 'pixels', ...
    'position', pos_btn_ca_next, 'string', '>>',...
    'callback', 'cell_id=cell_id+1; cb_show_2p_neuron;');

handles.btn_confidence = uicontrol(handles.fig_main, 'style', 'pushbutton', 'units', 'pixels', ...
    'position', pos_btn_confidence, 'string', 'confidence', 'fontweight', 'bold',...
    'fontsize', font_size, 'callback', 'neuron.evaluate_matching_confidence(Aem);');
handles.text_rate = uicontrol(handles.fig_main, 'style', 'text', 'units', 'pixels', ...
...
handles.btn_em_ahead = uicontrol(handles.fig_main, 'style', 'pushbutton', 'units', 'pixels', ...
    ...
    'callback', 'em_rank = max(1, em_rank-1); cb_check_match;');
...
switch lower(e.Key)
    ...
    case 'b' % check enxt cell
        evalin('base', 'cell_id = max(1, cell_id-1); cb_show_2p_neuron;');
    case '0' % label this neuron and delete it later
